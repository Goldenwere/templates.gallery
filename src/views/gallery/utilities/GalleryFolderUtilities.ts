...
/**
 * A set of utilities for managing folders in a gallery.
 */
export class GalleryFolderUtilities {
  /**
   ...
   */
  static getFolderDisplayName = (original: string, depth: number = 0) => {
    let padding = ''
   ...
   */
  static flattenFolders = (folders: folder[], depth: number = 0): object[] => {
    return folders.flatMap((other) => [
      {
        ...deepCopy(other),
        displayName: GalleryFolderUtilities.getFolderDisplayName(other.displayName || other.id, depth),
        // folders no longer needs to be set due to the flattening process
        folders: undefined,
        // depth replaces folders instead
        depth,
      },
      ...GalleryFolderUtilities.flattenFolders(other.folders || [], depth + 1)
    ])
  }
}